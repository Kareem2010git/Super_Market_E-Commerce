...
export default function Home() {
  ...
  useEffect(() => {
    // محاكاة أو انتظار تحميل المكونات والصور بالكامل
    const timer = setTimeout(() => {
      setLoading(false);
    }, 600); // مدة قصيرة لضمان ظهور اللودر بسلاسة
  ...
  if (loading) {
    return (
      <div dir="rtl" className="min-h-screen bg-[#f9f9ff] flex flex-col items-center justify-center space-y-4">
        <div className="w-12 h-12 border-4 border-[#facc15] border-t-transparent rounded-full animate-spin"></div>
        <p className="text-[#4d4632] font-medium text-sm">من فضلك انتظر لحظات ، حتى يتم تحضير الموارد ..</p>
      </div>
    );
  }
