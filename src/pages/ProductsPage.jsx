...
export default function ProductsPage() {
  ...
  const addToCart = (product) => {
    ...
    if (stockQty <= 0) {
      toast.error(`عذراً، منتج "${product.name}" نفد من المخزون!`);
      return;
    }
    ...
    if (existingIndex > -1) {
      if (cartItems[existingIndex].quantity >= stockQty) {
        toast.warning(`لا يمكن إضافة المزيد، لقد وصلت للحد الأقصى المتوفر في المخزون!`);
        return;
      }
                  ...
                  <div key={product.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col justify-between hover:shadow-md transition">
                    <div>
                      <div className="h-48 overflow-hidden bg-gray-100 flex items-center justify-center relative">
                        ...
                        <span className={`absolute top-3 left-3 px-2.5 py-1 rounded-full text-xs font-bold shadow-xs ${isOutOfStock ? 'bg-red-500 text-white' : 'bg-white/90 text-gray-800'}`}>
                          {isOutOfStock ? 'نفذت الكمية' : "متوفر"}
                        </span>
                      </div>
                    ...
                    <div className="p-4 pt-0 flex items-center justify-between mt-4">
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-sm font-medium transition flex items-center gap-1.5 shadow-sm ${isOutOfStock ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-amber-500 hover:bg-amber-600 text-white cursor-pointer'}`}
                      >
