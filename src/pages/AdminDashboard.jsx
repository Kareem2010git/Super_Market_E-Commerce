...
export default function AdminDashboard() {
  ...
  const [productCategory, setProductCategory] = useState('خضروات');
  ...
  const categoriesList = ['خضروات', 'فواكه', 'لحوم', 'ألبان', 'المعلبات', 'تمور'];
  ...
  return (
    <div dir="rtl" className="flex min-h-screen bg-surface text-on-surface relative">
      <ToastContainer position="bottom-left" autoClose={3000} rtl={true} />
      
      {actionLoading && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs z-50 flex flex-col items-center justify-center gap-3">
          <div className="w-12 h-12 border-4 border-primary border-t-transparent rounded-full animate-spin"></div>
          <p className="text-white font-bold text-sm bg-black/60 px-4 py-2 rounded-xl">جاري تنفيذ العملية، يرجى الانتظار...</p>
        </div>
      )}

      <aside className="hidden lg:flex h-[calc(100vh-5rem)] w-64 fixed right-0 top-20 bg-surface-container border-l border-outline-variant shadow-md z-30 flex-col justify-between">
        <div>
      ...
      <main className="flex-grow w-full lg:mr-64 pt-20 p-4 md:p-6 transition-all duration-300">
          ...
          <section className="lg:col-span-6 bg-white border border-outline-variant rounded-2xl p-4 md:p-6 shadow-sm">
            ...
            <form onSubmit={handleAddProduct} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-sm text-on-surface-variant px-1 font-semibold">الكمية (المخزون)</label>
                  <input 
                    type="number" 
                    min="0"
                    value={productStock}
              ...
              <button 
                ...
                className="w-full mt-2 py-3 rounded-xl bg-slate-900 text-white font-bold text-base shadow-md hover:opacity-90 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {uploadingProduct && <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>}
                <span>{uploadingProduct ? "جاري الرفع والحفظ..." : "حفظ المنتج الجديد"}</span>
              </button>
            </form>
          </section>
          ...
          <section className="lg:col-span-6 bg-white border border-outline-variant rounded-2xl p-4 md:p-6 shadow-sm">
            ...
            <form onSubmit={handleAddOffer} className="space-y-4">
              ...
              <button 
                ...
                className="w-full mt-2 py-3 rounded-xl bg-slate-900 text-white font-bold text-base shadow-md hover:opacity-90 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {uploadingOffer && <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>}
                <span>{uploadingOffer ? "جاري رفع البانر والنشر..." : "حفظ ونشر العرض"}</span>
              </button>
            </form>
          </section>
            ...
            <div>
              <div className="flex items-center justify-between mb-6 border-b border-outline-variant pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-lg md:text-xl font-bold text-on-surface">قائمة المنتجات </h2>
                </div>
              </div>
                ...
                <table className="w-full text-right min-w-[500px]">
                  <thead>
                    <tr className="text-on-surface-variant border-b border-outline-variant bg-surface-container-low text-xs md:text-sm font-semibold">
                      ...
                      <th className="p-3">الكمية (المخزون)</th>
                  ...
                  <tbody className="divide-y divide-outline-variant text-sm md:text-base">
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan="8" className="p-6 text-center text-on-surface-variant">لا توجد منتجات مضافة حتى الآن.</td>
                      </tr>
                    ) : (
          ...
          <section className="lg:col-span-12 bg-white border border-outline-variant rounded-2xl p-4 md:p-6 shadow-sm overflow-hidden flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-6 border-b border-outline-variant pb-4">
                <div className="flex items-center gap-3">
                  <span className="material-symbols-outlined text-primary">local_offer</span>
                  <h2 className="text-lg md:text-xl font-bold text-on-surface">قائمة العروض النشطة </h2>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleUpdateProduct} className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="text-sm font-semibold text-gray-700">الكمية</label>
                  <input 
                    type="number" 
                    min="0"
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button 
                  type="submit" 
                  disabled={updating}
                  className="px-4 py-2 rounded-xl bg-slate-900 text-white font-bold text-sm hover:opacity-90 disabled:opacity-50 flex items-center gap-2"
                >
                  {updating && <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>}
                  <span>{updating ? "جاري التحديث..." : "حفظ التغييرات"}</span>
                </button>
              </div>
            </form>
          </div>
